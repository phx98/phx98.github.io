...
// $('.content>div').height()
$('.subnav ul li').on("click", function (element) {
    $('.content').css({
        top: $(this).index() * -100 + '%',
    })
    $(this).addClass('hua').siblings('li').removeClass('hua')
})
let skillone = echarts.init(document.getElementById('skillone'));
...
let optionone = {
    ...
    xAxis: {
        data: ['JAVASCRIPT', 'HTML5', 'CSS3', 'JQUERY', 'VUE'],
        axisTick: { show: false },
        axisLine: { show: false },
...
function all() {
    var lhand = setTimeout(function () {
        context.beginPath();
        context.fillRect(80, 80, 20, 80);
        // context.fillStyle = 'blue';
        context.arc(90, 80, 10, Math.PI * 0, Math.PI * 2, true);
        context.fill();
        context.arc(90, 160, 10, Math.PI * 0, Math.PI * 1, false);
        context.fill();
        context.closePath();
    }, 500)
    var rhand = setTimeout(function () {
        context.beginPath();
        context.fillRect(180, 80, 20, 80);
        // context.fillStyle = 'blue';
        context.arc(190, 80, 10, Math.PI * 0, Math.PI * 2, true);
        context.fill();
        context.arc(190, 160, 10, Math.PI * 0, Math.PI * 1, false);
        context.fill();
        context.closePath();
    }, 1000)
    var lfoot = setTimeout(function () {
        context.beginPath();
        context.fillRect(100, 190, 20, 60);
        // context.fillStyle = 'blue';
        context.arc(110, 190, 10, Math.PI * 0, Math.PI * 2, true);
        context.fill();
        context.arc(110, 250, 10, Math.PI * 0, Math.PI * 1, false);
        context.fill();
        context.closePath();
    }, 1500)
    var rfoot = setTimeout(function () {
        context.beginPath();
        context.fillRect(160, 190, 20, 60);
        // context.fillStyle = 'blue';
        context.arc(170, 190, 10, Math.PI * 0, Math.PI * 2, true);
        context.fill();
        context.arc(170, 250, 10, Math.PI * 0, Math.PI * 1, false);
        context.fill();
        context.closePath();
    }, 2000)
    var head = setTimeout(function () {
        context.beginPath();
        context.arc(140, 70, 40, Math.PI * 0, Math.PI * -1, true);
        context.fill();
        context.closePath();

    }, 2500)
    var lear = setTimeout(function () {
        context.beginPath();
    ...
    var leye = setTimeout(function () {
        context.beginPath();
        context.fillStyle = '#fff';
        context.arc(120, 50, 5, Math.PI * 0, Math.PI * 2, false);
        context.fill();
        context.closePath();
    }, 4000)
    var reye = setTimeout(function () {
        context.beginPath();
        context.arc(160, 50, 5, Math.PI * 0, Math.PI * 2, false);
        context.fill();
        context.closePath();
    }, 4500)
    var shen = setTimeout(function (params) {
        context.fillStyle = 'green';
        context.beginPath();
        context.fillRect(105, 80, 70, 80);
        context.closePath();
        context.beginPath();
        context.moveTo(120, 160);
        context.arc(120, 160, 15, Math.PI * 1 / 2, Math.PI * 1, false);
        context.fill();
        context.closePath();

        context.beginPath();
        context.moveTo(160, 160);
        context.arc(160, 160, 15, Math.PI * 0, Math.PI * 1 / 2, false);
        context.fill();
        context.closePath();
        context.beginPath();
        context.fillRect(120, 160, 40, 15);
        context.closePath();
    }, 5000)
}
...
window.onload = function () {
    var arr = ['0px 0px', '60px 0px', '30px 0px', '0px 60px', '60px 60px', '30px 60px', '0px 30px',
        '60px 30px', '30px 30px']
    var box = document.querySelector('.boxfour')
    var span = document.querySelectorAll('.boxfour p span')
    var puz = new Nine(span, arr, box)
}

function Nine(span, arr, box) {
    this.span = span
    this.arr = arr
    this.box = box
    this.arrNew = []
...
// 遍历并且打乱顺序
Nine.prototype.one = function () {
    var tem = copy(this.arr)
    var that = this
    this.span.forEach(function (sp, index) {
        // console.log(this.style.backgroundPosition)
        var num = Math.floor(Math.random() * tem.length)
        sp.style.backgroundPosition = tem[num]
        that.arrNew.push(tem[num])
        tem.splice(num, 1)
...
// 接下来,就是可以互换位置了,用拖放事件来解决

Nine.prototype.two = function () {
    var that = this
    this.span.forEach(function (sp, index) {
        sp.draggable = true
        sp.ondragstart = function (event) {
            // console.log('开始拖动')
            // dataTransfer 数据传输者
            var arr = [this.style.backgroundPosition, index]
            console.log(arr);
            event.dataTransfer.setData('Text', arr)
            // 没有使用事件委托,就是一样的,不然就是不一样的
        }
        ...
        sp.ondrop = function (e) {
            ...
            var i = data.split(',', 2)
            for (var j = 0; j < that.span.length; j++) {
                // console.log(that.span[j]);
                if (that.span[j].style.backgroundPosition == i[0] && j != i[1]) {
                    console.log('不可以')
                    return;
                }

            }
            var str = this.style.backgroundPosition
            that.span[i[1]].style.backgroundPosition = str
            this.style.backgroundPosition = i[0]
            //    console.log(that.span[i].style.backgroundPosition)
        }
    })
}
